...
export const Background = ({userId} : {userId:string}) => {
    ...
    return (
      ...
      <div className=' z-10  flex flex-col justify-center items-center p-4'>
          ...
          <button onClick={() => router.push(`${userId ?'/client' :'/sign-in' }`)} className='mt-10 md:mt-5  hover:opacity-90  px-8 nft-gradient  text-sm font-semibold py-3 text-nft-gray-3  rounded-md'>Get started !</button>
      </div>
    </div>}
      </>
    )
